...
def scanner_universe(limit:int|None=None):
    frame=load_top_us_stocks(limit); return frame[~frame['ticker'].isin(EXCLUDED_TICKERS)].reset_index(drop=True)
...
def write_web_output(result,universe_size:int,path:str)->None:
    ...
    payload={'engine_version':2,'generated_at':datetime.now(timezone.utc).isoformat(),'universe_size':int(universe_size),'alert_count':int((result['action']=='BUY').sum()) if not result.empty else 0,'top_score':top_score,'results':rows}
...
def main()->None:
    parser=argparse.ArgumentParser(description='MarketIntel Opportunity Engine scanner'); sub=parser.add_subparsers(dest='command',required=True)
    ...
    if args.command=='scan':
        scanner_module.load_top_us_stocks=scanner_universe; universe=scanner_universe(args.limit); result=scan(args.limit,args.top); print(result.to_string(index=False))
