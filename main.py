...
def write_web_output(result, universe_size: int, path: str) -> None:
    ...
    top_score = None
    if not result.empty and "opportunity_score" in result:
        top_score = _json_safe(result["opportunity_score"].max())
    payload = {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MarketIntel opportunity scanner")
    ...
    v = sub.add_parser("validate-market", help="validate the opportunity engine")
