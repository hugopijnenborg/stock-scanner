...
def write_web_output(result, universe_size: int, path: str) -> None:
    ...
    top_score = _json_safe(result['opportunity_score'].max()) if not result.empty and 'opportunity_score' in result else None
    payload = {
        'generated_at': datetime.now(timezone.utc).isoformat(),
        'universe_size': int(universe_size),
        'alert_count': int((result['action'] == 'BUY').sum()) if not result.empty and 'action' in result else 0,
        'top_score': top_score,
        'results': rows,
    }
    output = Path(path)
...
def main() -> None:
    ...
    s = sub.add_parser('scan', help='scan current market with Opportunity Engine')
    ...
    v = sub.add_parser('validate-market', help='validate the Opportunity Engine output')
