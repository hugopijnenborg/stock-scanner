...
def run_backtest(path: str = TRADER_HISTORY_PATH) -> pd.DataFrame:
    ...
    for _, entry in entries.iterrows():
        ...
        matched_date = eligible.index[-1]
        entry_pos = close.index.get_indexer([matched_date])[0]
        result = {**entry.to_dict(), "data_available": True, "matched_session": matched_date, "market_close": float(row["Close"]), "entry_vs_close": actual_entry_price / float(row["Close"]) - 1}
        result.update(score)
        ...
        entry_day = matched_date.normalize()
        ...
        if target_positions:
            ...
            result["return_60d"] = None; result["max_gain_60d"] = None; result["max_drawdown_60d"] = None
        output.append(result)
    return pd.DataFrame(output)

if __name__ == "__main__":
    df = run_backtest(); print(df.to_string(index=False)); df.to_csv("trader_backtest.csv", index=False)
