...
export function buildTradePlan(position, result) {
  ...
  const technicalTargets = resistances;

  let firstTarget = technicalTargets[0] || null;
  let secondTarget = null;

  if (!firstTarget && analystTarget && analystTarget > price * 1.02) {
    firstTarget = analystTarget;
  }

  if (firstTarget) {
    secondTarget = analystTarget && analystTarget > firstTarget * 1.03 ? analystTarget : null;
    if (!secondTarget) {
      const nextResistance = technicalTargets.find((level) => level > firstTarget * 1.03);
  ...
  if (!secondTarget && analystTarget && analystTarget > price * 1.02 && analystTarget !== firstTarget) {
    secondTarget = analystTarget;
  }
  ...
  const safeStop = Math.min(stop, price * 0.97);
  const riskPct = price > safeStop ? 1 - safeStop / price : null;

  const addTrigger = support
    ? Math.min(support * 1.02, price * 0.99)
    : price * Math.max(0.94, 1 - atrPct * 0.5);
  const addLow = support ? Math.max(safeStop, support * 0.98) : safeStop;
  const addHigh = Math.max(addLow, addTrigger);
  ...
  if (price <= addHigh * 1.01) context = 'Koers zit in of vlak boven de technische koopzone rond steun.';
...
export function getPortfolioAction(position, result, plan = buildTradePlan(position, result)) {
  ...
  if (nearStop) {
    return { label: 'SELL', tone: 'sell', reason: 'De koers heeft de vooraf bepaalde risicogrens bereikt. Bescherm het kapitaal.' };
  }

  if (score != null && score < 60) {
    return { label: 'SELL', tone: 'sell', reason: 'De totaalscore is onder 60 gezakt. De oorspronkelijke setup is te zwak geworden.' };
  }

  if (scoreChange != null && scoreChange <= -10) {
    return { label: 'SELL', tone: 'sell', reason: 'De totaalscore is minimaal 10 punten verslechterd sinds aankoop.' };
  }

  if (analystTargetReached && (gain == null || gain > 0.05)) {
    return { label: 'SELL', tone: 'sell', reason: 'Het gemiddelde analistenkoersdoel is vrijwel bereikt. Winst nemen is nu rationeler dan verder bijkopen.' };
  }

  if (secondTargetReached && gain != null && gain > 0.05) {
    return { label: 'SELL', tone: 'sell', reason: 'Het tweede koersdoel is bereikt. Overweeg winst te nemen of de positie gedeeltelijk af te bouwen.' };
  }
  ...
  if (strongSetup && inAddZone && enoughUpside && acceptableRiskReward && notTooCloseToTarget && scoreHealthy) {
    return {
      label: 'ADD',
      tone: 'add',
      reason: `Sterke score en technische bevestiging. De koers zit in de koopzone met ${rewardRisk ? `${rewardRisk.toFixed(1)}x` : 'voldoende'} risk/reward.`,
    };
  }
  ...
  if (strongSetup && !inAddZone) {
    return { label: 'HOLD', tone: 'hold', reason: 'De setup blijft sterk, maar de koers staat buiten de optimale koopzone. Wacht liever op een betere instap.' };
  }

  return {
    label: 'HOLD',
    tone: 'hold',
    reason: 'De positie blijft binnen de normale risicobandbreedte. Geen duidelijke ADD- of SELL-trigger.',
  };
}
