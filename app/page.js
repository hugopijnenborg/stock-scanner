...
import { RefreshCw, Bell, Search, Activity, ArrowUpRight, Database, BarChart3, X, TrendingUp, ShieldCheck, LineChart, History, LayoutDashboard, Target, Layers3, ChevronRight, CircleDot } from 'lucide-react';
...
function signalLabel(r) {
  if (r?.signal === 'ALERT') return 'BUY ALERT';
  if (r?.watch_candidate) return 'WATCH';
  return 'NO SIGNAL';
}
...
function Detail({ r, onClose }) {
  ...
  return <div className="modalBackdrop" onClick={onClose}>
    <div className="detailPanel" onClick={e => e.stopPropagation()}>
      ...
      <div className="detailScore"><div><span>TOTAALSCORE</span><strong>{score(r.overall_score)}<small>/100</small></strong></div><span className={`signal ${r.watch_candidate ? 'watch' : String(r.signal).toLowerCase()}`}>{signalLabel(r)}</span></div>
      ...
      <div className="detailSection"><h3>Watch status</h3><div className="scoreMethod"><span>Dip score</span><b>{score(r.dip_score)} / 100</b><small>{r.watch_candidate ? 'Dicht bij een echte buy alert en voldoet aan de dip/oversold watch-criteria.' : 'Geen watch candidate op dit moment.'}</small></div></div>
      <div className="detailSection"><h3>Analisten & koersdoelen</h3><AnalystPanel r={r} /></div>
      <div className="detailSection"><h3>Technische data</h3><div className="detailGrid"><div><span>RSI 14</span><b>{num(r.rsi_14)}</b></div><div><span>Dip score</span><b>{score(r.dip_score)}</b></div><div><span>MACD histogram</span><b>{num(r.macd_histogram, 3)}</b></div><div><span>Volume ratio</span><b>{num(r.volume_ratio, 2)}x</b></div><div><span>ATR</span><b>{pct(r.atr_pct)}</b></div><div><span>Onder 52W high</span><b>{pct(r.distance_52w_high)}</b></div><div><span>vs SMA20</span><b>{pct(r.distance_sma20)}</b></div><div><span>vs SMA50</span><b>{pct(r.distance_sma50)}</b></div><div><span>Rel. strength</span><b>{pct(r.relative_strength_20d)}</b></div></div></div>
...
function Sidebar({ alerts, watch }) {
  return <aside className="sidebar">
    ...
    <div className="sideBottom"><div className="liveStatus"><span className="liveDot" />MARKTSCANNER ACTIEF</div><div className="sideStat"><span>ACTIEVE ALERTS</span><strong>{alerts.length}</strong></div><div className="sideStat"><span>WATCH CANDIDATES</span><strong>{watch.length}</strong></div></div>
  </aside>;
}
...
export default function Home() {
  ...
  const watch = useMemo(() => all.filter(r => r.watch_candidate === true && r.signal !== 'ALERT').sort((a, b) => Number(b.overall_score) - Number(a.overall_score)), [all]);
  const opps = useMemo(() => all.filter(r => r.signal !== 'ALERT' && !r.watch_candidate).sort((a, b) => Number(b.overall_score) - Number(a.overall_score)).slice(0, 10), [all]);
  ...
  const top = alerts[0] || watch[0] || opps[0];
  ...
  return <div className="appShell">
    <Sidebar alerts={alerts} watch={watch} />
    <main className="dashboard">
      ...
      <section className="sectionBlock"><div className="sectionHeader"><div><div className="sectionEyebrow">02 · EARLY SIGNAL</div><h2>Watch candidates</h2><p>Geen buy alerts, maar wel duidelijke dip-, oversold- en trader-overlap. Dit zijn de aandelen die richting de alertzone bewegen.</p></div><div className={`statusPill ${watch.length ? 'live' : ''}`}><Target size={13} />{watch.length} watch</div></div>{watch.length === 0 ? <div className="emptyState"><div className="emptyIcon"><Target size={20} /></div><strong>Geen watch candidates op dit moment</strong><span>De huidige scan heeft geen kandidaat die tegelijk voldoende trader-overlap, technische dip-score en oversold-profiel heeft.</span></div> : <div className="opportunityGrid">{watch.slice(0, 10).map((r, i) => <button className="opportunity opportunityButton" key={r.ticker} onClick={() => setSelected(r)}><div className="rank">{String(i + 1).padStart(2, '0')}</div><div className="oppMain"><div><strong>{r.ticker}</strong><span>{r.company_name}</span></div><span className="badge">WATCH</span></div><div className="oppScore"><strong>{score(r.overall_score)}</strong><small>/100</small></div><div className="oppStats"><span>TRADER <b>{score(r.trader_similarity_score)}</b></span><span>TECH <b>{score(r.technical_score)}</b></span><span>DIP <b>{score(r.dip_score)}</b></span></div></button>)}</div>}</section>

      <section className="sectionBlock"><div className="sectionHeader"><div><div className="sectionEyebrow">03 · RESEARCH</div><h2>Top opportunities</h2><p>De sterkste kandidaten onder de alertdrempel die nog niet aan de watch-criteria voldoen.</p></div><Activity size={18} /></div><div className="opportunityGrid">{opps.map((r, i) => <button className="opportunity opportunityButton" key={r.ticker} onClick={() => setSelected(r)}><div className="rank">{String(i + 1).padStart(2, '0')}</div><div className="oppMain"><div><strong>{r.ticker}</strong><span>{r.company_name}</span></div><span className="badge">{r.setup_type}</span></div><div className="oppScore"><strong>{score(r.overall_score)}</strong><small>/100</small></div><div className="oppStats"><span>TRADER <b>{score(r.trader_similarity_score)}</b></span><span>TECH <b>{score(r.technical_score)}</b></span><span>ANALYST <b>{score(r.analyst_score)}</b></span></div></button>)}</div></section>

      <section className="sectionBlock databaseBlock"><div className="sectionHeader"><div><div className="sectionEyebrow">04 · MARKET UNIVERSE</div><h2>Alle gescoorde bedrijven</h2><p>De volledige vaste universe, gerangschikt op alertscore.</p></div><div className="search"><Search size={15} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Zoek ticker of bedrijf..." /></div></div><div className="tablewrap"><table><thead><tr><th>#</th><th>BEDRIJF</th><th>SIGNAAL</th><th>SCORE</th><th>TRADER</th><th>TECHNICAL</th><th>ANALYST</th><th>FUNDAMENTALS</th><th>OMZETGROEI</th><th>EPS GROEI</th><th>P/E</th></tr></thead><tbody>{(showAll ? filtered : filtered.slice(0, 20)).map((r, i) => <tr key={r.ticker} onClick={() => setSelected(r)} className="clickRow"><td className="muted">{String(i + 1).padStart(2, '0')}</td><td><div className="company"><span className="ticker">{r.ticker}</span><small>{r.company_name}</small></div></td><td><span className={`signal ${r.watch_candidate ? 'watch' : String(r.signal).toLowerCase()}`}>{signalLabel(r)}</span></td><td><div className="tableScore"><b>{score(r.overall_score)}</b><i><em style={{ width: `${Math.min(100, Number(r.overall_score) || 0)}%` }} /></i></div></td><td>{score(r.trader_similarity_score)}</td><td>{score(r.technical_score)}</td><td>{score(r.analyst_score)}</td><td>{score(r.fundamental_score)}</td><td>{pct(r.revenue_growth)}</td><td>{pct(r.eps_growth)}</td><td>{num(r.pe)}</td></tr>)}</tbody></table></div><button className="showMore" onClick={() => setShowAll(!showAll)}>{showAll ? 'Toon minder' : `Toon alle ${filtered.length} bedrijven`} <ArrowUpRight size={13} /></button></section>

      {selected && <Detail r={selected} onClose={() => setSelected(null)} />}
      <section className="footerInfo"><div><Database size={17} /><div><strong>Hoe de score werkt</strong><p>50% trader pattern, 30% technische setup en 20% analisten. Fundamentals worden niet gebruikt voor de alertscore.</p></div></div><div><Target size={17} /><div><strong>Watch candidates</strong><p>Een watch candidate is nog geen buy alert. Hij combineert trader-overlap met een duidelijke dip- en oversold-situatie en zit onder de alertdrempel.</p></div></div></section>
