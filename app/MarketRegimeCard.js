...
export default function MarketRegimeCard() {
  useEffect(() => {
    ...
    const render = async () => {
      ...
      if (!heroStats) { timer = setTimeout(render, 1000); return; }
      try {
        ...
        if (!card) { card = document.createElement('div'); card.className = 'marketRegimeStat'; heroStats.appendChild(card); }
    ...
    return () => { alive = false; clearInterval(interval); clearTimeout(timer); };
  }, []);
  return null;
}
