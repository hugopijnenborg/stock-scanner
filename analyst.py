...
def _parse_changes(df):
    ...
    for idx, row in d.head(60).iterrows():
        ...
        action, firm = _clean_text(row.get("action")), _clean_text(row.get("firm"))
        to_grade, from_grade = _clean_text(row.get("toGrade")), _clean_text(row.get("fromGrade"))
        ...
        if len(changes) < 6: changes.append({"date": ts.isoformat() if ts is not None else None, "firm": firm, "action": action, "from_grade": from_grade, "to_grade": to_grade, "target": target})
        if firm and firm not in seen:
            seen.add(firm)
            firms.append({"firm": firm, "date": ts.isoformat() if ts is not None else None, "action": action, "from_grade": from_grade, "to_grade": to_grade, "target": target})
...
def _has_analyst_data(row):
    ...
    return any([
        ...
        any((row.get(k) or 0) > 0 for k in ("analyst_strong_buy","analyst_buy","analyst_hold","analyst_sell","analyst_strong_sell")),
...
def _one(ticker):
    ...
    try:
        ...
        mean=targets.get("mean") or _num(info.get("targetMeanPrice")); median=targets.get("median") or _num(info.get("targetMedianPrice")); low=targets.get("low") or _num(info.get("targetLowPrice")); high=targets.get("high") or _num(info.get("targetHighPrice"))
        ...
        total_ratings=sum(counts.values()); info_count=_num(info.get("numberOfAnalystOpinions")); analyst_count=total_ratings or (int(info_count) if info_count else None)
        if consensus is None and recommendation_key:
            consensus=RATING_WEIGHTS.get(str(recommendation_key).replace(" ","").replace("_","").lower())
