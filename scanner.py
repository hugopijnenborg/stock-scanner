...
from config import ALERT_THRESHOLD, DEFAULT_START, MIN_AVG_DOLLAR_VOLUME, MIN_PRICE, REBOUND_WEIGHTS, QUALITY_WEIGHTS, CYCLICAL_WEIGHTS
...
FUNDAMENTAL_COLUMNS = ["revenue", "revenue_growth", "eps", "eps_growth", "net_margin", "gross_margin", "fcf", "fcf_growth", "roe", "debt_equity", "cash", "pe", "forward_pe", "peg", "fundamental_score"]
...
def scan(limit: int = 1000, top_n: int = 25) -> pd.DataFrame:
    ...
    for ticker, prices in market.items():
        ...
        technical_score = float(scores["overall_score"])
        fundamental_score = f.get("fundamental_score")
        combined_score = technical_score if fundamental_score is None else (technical_score * 0.70 + float(fundamental_score) * 0.30)
        result = {"ticker": ticker, "company_name": company_map.get(ticker, ticker), "price": float(row["Close"]), "avg_dollar_volume_20d": float(avg_dollar_volume) if pd.notna(avg_dollar_volume) else None, "technical_score": round(technical_score, 1), "fundamental_score": fundamental_score, "overall_score": round(combined_score, 1)}
        result.update({k: float(v) if isinstance(v, (int, float)) else v for k, v in scores.items() if k != "overall_score"})
        result["signal"] = _signal_label(combined_score)
    ...
    return pd.DataFrame(rows).sort_values(["overall_score", "reversal_trigger"], ascending=[False, False]).head(top_n).reset_index(drop=True)
